...
int decrypt(char *text, char *key, char first, char firstkey, int size) {
//******************************************************************************
// FUNCTION NAME: decrypt
// AUTHOR: Ryan Sisco
// PURPOSE: decrypts the data with the key
	...
	for (l = 0; l < 5; l++) {
		...
		if (pid == 0) {	// if successful fork
			...
			while (1) {
				...
				char firstkey = key[0];
				int charCount = decrypt(text, key, first, firstkey, sizeof(text)); //gets number of characters
